...
export default class UserBusiness {
    public signup = async (imput: any) => {

        const { name, email, password } = (imput)
    ...
    public login = async (imput: any) => {
        const { email, password } = (imput)
        if (!email || !password) {
            throw new Error('Todos os campos devem estar preeenchidos.')
        }
    ...
    public deleteUserById = async (token: string, idForDelelte: string) => {
