...
// desenvolver o sistema de organização de estoque da empresa. A pessoa 
// desenvolvedora anterior a você chegou a criar uma função que ajusta os preços 
...
const ajustaPreco = (produto: produtos[]) => {
    // console.log(produto)
    const teste: (number | string)[] = produto.map((prod) => {
        return prod.valorUnitario.toFixed(2)
    })
